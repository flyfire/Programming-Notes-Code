...
此时如果我们不调用proceed方法，被切入的方法将不会被调用,Around和After是不能同时作用在同一个方法上的，会产生重复切入的问题。
    ...
    public void testMainActivityAround(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        Log.d(proceedingJoinPoint.getThis().getClass().getSimpleName(),
                "testMainActivityAround() called with: joinPoint = [" + proceedingJoinPoint + "]");
    }
   ...
   自定义Pointcuts可以让我们更加精确的切入一个或多个指定的切入点。
    ...
    /*
    call：call与execution的区别在于，execution是把切入的代码调用放在方法内部，而call则是把切入的代码的调用放在被切入分发的外围
     */
    ...
    public void testMainActivityAroundCall(ProceedingJoinPoint joinPoint) throws Throwable {
        String key = joinPoint.getSignature().toString();
        Log.d(joinPoint.getThis().getClass().getSimpleName(),
                "testMainActivityAroundCall() called with: joinPoint = [" + key + "]");
    }
