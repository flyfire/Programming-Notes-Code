...
- 指针也是一种变量，占有内存空间，用来保存内存地址，32为系统指针占四个字节，64位系统指针占8个字节
- `*p`操作内存
...
- 指针变量和它指向的内存块是两个不同的概念
    ...
    - 保证所指的内存块是能修改的
- 指针是一种数据类型，是指它指向的内存空间的数据类型：**指针步长（p++），根据所致内存空间的数据类型来确定**
    - `int *p;  p++;` 移动了4个字节
    - `char *p; p++;` 移动了1个字节
...
一级指针做输入
```c
    int showbuf(char *p)//以0结尾的字符数组可以不传数组长度
...
二级指针做输出
```c
int GetTeacher(Teacher **ppTeacher);//一般使用指针操作结构体，
int GetTeacher_Free(Teacher **ppTeacher);
int getData(char **data, int *dataLen);//返回字符数组还有长度
