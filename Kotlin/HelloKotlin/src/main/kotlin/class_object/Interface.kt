...
/**
 * 定义接口：Kotlin 的接口与 Java 8 类似，既包含抽象方法的声明，也包含 实现。
 *1，接口无法保存状态。它可以有 属性但必须声明为抽象或提供访问器实现。
 *2，一个类或者对象可以实现一个或多个接口。
 *3，可以在接口中定义属性。在接口中声明的属性要么是抽象的，要么提供 访问器的实现。
 *      在接口中声明的属性不能有幕后字段（backing field），因此接口中声明的访问器 不能引用它们。
 *
 */
...
class ImplAB : InterfaceA, InterfaceB {
