...
/**
 * 幕后字段：Kotlin 中类不能有字段。然而，当使用自定义访问器时，有时有一个幕后字段（backing field）有时是必要的。
 *                 为此 Kotlin 提供 一个自动幕后字段，它可通过使用 field 标识符访问。
 *
 * 1，如果属性至少有一个访问器使用默认实现，或者自定义访问器通过 field 引用幕后字段，将会为该属性生成一个幕后字段。
 *
...
class BackingField {
    ...
    var counter = 0 // 此初始器值直接写入到幕后字段
        set(value) {
            if (value >= 0)
                field = value//field 标识符只能用在属性的访问器内。
        }
...
/**
 * 延迟初始化属性：一般地，属性声明为非空类型必须在构造函数中初始化。可以用 lateinit 修饰符标记该属性来实现延迟舒适化
 *
 *1，该修饰符只能用于在类体中（不是在主构造函数中）声明的 var 属性，
 *     并且仅 当该属性没有自定义 getter 或 setter 时。该属性必须是非空类型，并且不能是 原生类型。
...
fun testLateinit() {
    val testLateinit = TestLateinit()
    testLateinit.subject = "abc"
    println(testLateinit.subject)
}
