 ...
 *
 *1，虽然密封类也可以 有子类，但是所有子类都必须在与密封类自身相同的文件中声明。
...
data class Const(val number: Double) : SealedExpr()
data class Sum(val e1: SealedExpr, val e2: SealedExpr) : SealedExpr()
object NotANumber : SealedExpr()

fun eval(expr: SealedExpr): Double =
        when (expr) {
